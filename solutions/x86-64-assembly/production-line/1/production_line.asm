...
calibrate_batch:
    ...
    ; The function has no return value
    cvtps2pd xmm0, [rdi]
    subpd xmm0, [rdx] ; value - offset
    movapd xmm1, [rsi]
    divpd xmm1, xmm0 ; ref / (value - offset)
    mulpd xmm1, [half]
    movapd oword [rcx], xmm1 
...
normalize_scores:
    ...
    mov r8, 0 ; index
.loop:
    movapd xmm0, oword [rdi + r8 * 8 + 0] ; scores
    mulpd xmm0, [rsi + r8 * 8 + 0] ; value * gain
    divpd xmm0, [rdx] ; value * gain / scale
    movapd oword [rdi + r8 * 8 + 0], xmm0
    add r8, 2
    cmp r8, rcx
    jge .end
    jmp .loop
